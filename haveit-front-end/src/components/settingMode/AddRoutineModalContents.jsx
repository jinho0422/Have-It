...
export default class AddRoutineModalContents extends Component {
	state = {
		startHour: null,
		startMin: null,
		weekId: 0,
		...
		endHour: null,
		endMin: null,
		dollId: window.sessionStorage.getItem('dollId0'),
		isAmStart: 1,
		isAmEnd: 1,
		error: null,
	}
	...
	hourList(max) {
		...
		for (let i=1; i<=max; i++) ret.push(<option>{i}</option>)
		return ret;
	}
	...
	requestAndGetMessage = async (habitId, time, weekId, repeat, endTime, dollId) => {
		try {
			await API.postHabitDetail({ habitId, time, weekId, repeat, endTime, dollId });
		} catch (e) {
			if (e.response.data.message) {
				alert(e.response.data.message)
			} else {
				alert('알람이 생성되었으나 인형과의 연결이 불안정합니다. 회원 정보 관리창에서 인형 동기화 버튼을 눌러주세요.')
			}
		}
	};
	...
	handleSubmit(e) {
		...
		if (Flag){
			this.requestAndGetMessage(this.props.habitId, totalStartTime, this.state.weekId, this.state.repeat, totalEndTime, this.state.dollId)
			this.props.onHide();
		};
								...
								<Container className="justify-content-md-center">
									...
									<Row className="justify-content-md-center"> 
										<Form.Group style={{margin:5}}>
											<Button type="submit" /*onClick={function() {this.props.changeSettingList()}.bind(this)}*/>저장</Button>
										</Form.Group>
										<Form.Group style={{margin:5}}>
